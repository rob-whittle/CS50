 ...
 ***************************************************************************/
 ...
 int main(void)
 {
    ...
    // Loop over card a block at a time, each time checking that a whole block is succesffuly read in   
    while (fread(buffer, BLOCK, 1, card) == 1) // fread returns number of succesffuly read blocks, so if not 1 then must be EOF
    {
