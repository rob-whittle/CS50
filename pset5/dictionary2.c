...
bool load(const char* dictionary)
{
    ...
    while (fscanf(inptr,"%s", buffer) != EOF)
        ...
        for (int i = 0; buffer[i] != '\0'; i++) 
            {
            buffer[i] = buffer[i] - 'a';  // convert char in buffer to an integer, such that zero represents 'a', could put some code in to deal with uppercase but no need for this dictionary
