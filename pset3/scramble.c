...
int main(int argc, string argv[])
{
    ...
    while (true)
    {
        ...
        // ensure word is in capitals - this must be placed after the the check for NULL, otherwise 'toupper' will alter bit pattern of NULL, or something...
...
void initialize(void)
{
    ...
    int n = sizeof(frequencies) / sizeof(float);  // length of frequencies array?? 
...
bool load(string s)
{
    ...
    while (fgets(buffer, LETTERS + 2, file))
    {
        ...
        dictionary.words[dictionary.size].found = false;    //set found to FALSE for every word in dictionary array
...
bool lookup(string s)
{
    ...
    //all letters in dictionary are same case so should be ok to treat as numbers
    ...
    while (begin <= end)
    {
        ...
        if (strcmp(s, dictionary.words[midpoint].letters) == 0)         // check if its in the dictionary
        {
            if (!dictionary.words[midpoint].found)                      // now check if its been found before or not
                {
                //printf("%d\n",dictionary.words[midpoint].found);
                //printf("%s\n",dictionary.words[midpoint].letters);
...
void scramble(void)
{
    ...
    char newgrid[DIMENSION][DIMENSION]; //new grid to hold scrambled elements
    
    for (int row = 0; row < DIMENSION; row++)
    {
        for (int col = 0; col < DIMENSION; col++)
        {
            newgrid[row][col] = grid[DIMENSION - 1 - col][row];
        }           
    }
    
    // copy newgrid to oldgrid
    for (int row = 0; row < DIMENSION; row++)
    {
        for (int col = 0; col < DIMENSION; col ++)
